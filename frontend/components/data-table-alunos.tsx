...
//       <span
//         className={`inline-block size-2.5 rounded-full ${cores[nivel] ?? "bg-gray-400"}`}
//       />
...
//   const acadAlto = aluno.Risco_Academico === "Alto";
//   const evasaoAlto = aluno.Risco_Evasao === "Alto";

//   if (acadAlto && evasaoAlto) {
//     return {
//       titulo: "🔴 Intervenção Urgente",
//       descricao:
//         "Este aluno apresenta risco crítico nos dois índices. Contato imediato da coordenação é recomendado — avalie encaminhamento para suporte pedagógico e psicossocial.",
//     };
//   }
//   if (!acadAlto && evasaoAlto) {
//     return {
...
//       descricao:
//         "As notas estão dentro do esperado, mas o aluno apresenta sinais silenciosos de desengajamento. Recomenda-se contato de mentoria para fortalecer o vínculo com a instituição.",
//     };
//   }
//   if (acadAlto && !evasaoAlto) {
//     return {
...
//       descricao:
//         "O aluno está engajado, mas enfrenta dificuldades de desempenho. Encaminhe para sessões de reforço ou tutoria acadêmica.",
//     };
//   }
//   return {
//     titulo: "🟢 Saudável",
//     descricao:
//       "Nenhum fator de risco relevante identificado. Manter o monitoramento regular.",
//   };
// }

// // --- Radar Chart do aluno ---
// const chartConfig: ChartConfig = {
//   valor: { label: "Score", color: "var(--chart-1)" },
// };

// function RadarAluno({ aluno }: { aluno: Aluno }) {
//   const data = [
//     { eixo: "Risco Geral", valor: aluno.Probabilidade_Problema },
//     { eixo: "Acadêmico", valor: aluno.Score_Academico },
//     { eixo: "Evasão", valor: aluno.Score_Evasao },
//     { eixo: "Frequência", valor: 100 - aluno.Frequencia },
//     { eixo: "Nota Exame", valor: 100 - aluno.Nota_Exame },
//   ];
...
//           <Input
//             placeholder="Filtrar por motivação..."
//             value={
//               (table
//                 .getColumn("Nivel_Motivacao")
//                 ?.getFilterValue() as string) ?? ""
//             }
//             onChange={(e) =>
//               table.getColumn("Nivel_Motivacao")?.setFilterValue(e.target.value)
//             }
...
//             {table.getFilteredRowModel().rows.length} alunos
//           </div>
...
//                 {/* Radar */}
//                 <RadarAluno aluno={alunoSelecionado} />
...
//                     </div>
//                     <RiscoBadge nivel={alunoSelecionado.Nivel_Risco} />
//                   </div>
//                   <div className="rounded-lg border p-3">
//                     <div className="text-xs text-muted-foreground mb-1">
//                       Acadêmico
...
//                     </div>
//                     <RiscoBadge nivel={alunoSelecionado.Risco_Academico} />
//                   </div>
//                   <div className="rounded-lg border p-3">
//                     <div className="text-xs text-muted-foreground mb-1">
//                       Evasão
...
//                     </div>
//                     <RiscoBadge nivel={alunoSelecionado.Risco_Evasao} />
//                   </div>
//                 </div>
...
//                     ["Motivação", alunoSelecionado.Nivel_Motivacao],
//                     ["Apoio Familiar", alunoSelecionado.Envolvimento_Pais],
//                   ].map(([label, valor]) => (
...
//       titulo: "🔴 Intervenção Urgente",
//       descricao:
//         "Risco crítico nos dois índices. Contato imediato da coordenação — avalie encaminhamento para suporte pedagógico e psicossocial.",
//     };
//   if (!acadAlto && evasaoAlto)
//     return {
//       titulo: "🟠 Risco de Evasão",
//       descricao:
//         "Notas ok, mas sinais silenciosos de desengajamento. Recomenda-se mentoria para fortalecer o vínculo com a instituição.",
//     };
//   if (acadAlto && !evasaoAlto)
//     return {
//       titulo: "🟡 Dificuldade Acadêmica",
...
//     { eixo: "Evasão", valor: aluno.Score_Evasao },
//     { eixo: "Frequência", valor: 100 - aluno.Frequencia },
//     { eixo: "Nota", valor: 100 - aluno.Nota_Exame },
//   ];
//   return (
//     <ChartContainer
//       config={chartConfig}
...
import { ChartRadialLabel, type RadialItem } from "@/components/chart-radial-label";
import { InfoIcon } from "lucide-react";
...
const TOOLTIPS: Record<string, string> = {
  ID_Aluno:               "Identificador único do aluno",
  Nivel_Risco:            "Índice geral de vulnerabilidade — combina fatores acadêmicos e de evasão",
  Probabilidade_Problema: "Score geral de risco em % — quanto maior, mais fatores de risco acumulados",
  Risco_Academico:        "Risco baseado em frequência, notas, reforço e dificuldade de aprendizado",
  Score_Academico:        "Score acadêmico em % — reflete desempenho e presença nas aulas",
  Risco_Evasao:           "Risco baseado em motivação, apoio familiar, distância e vínculo institucional",
  Score_Evasao:           "Score de evasão em % — quanto maior, mais sinais silenciosos de desengajamento",
  Frequencia:             "Percentual de presença nas aulas",
  Nota_Exame:             "Nota obtida no exame (0-100)",
  Nivel_Motivacao:        "Nível de motivação — Baixo aumenta risco de evasão",
  Envolvimento_Pais:      "Grau de envolvimento dos pais — Baixo aumenta risco de evasão",
  Distancia_Casa:         "Distância entre a residência e a instituição — Longe aumenta risco de evasão",
  Acesso_Recursos:        "Disponibilidade de materiais de estudo — Baixo aumenta risco de evasão",
  Atividades_Extra:       "Participação em atividades extracurriculares — protege contra evasão",
};
...
function RiscoBadge({ nivel }: { nivel: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={`inline-block size-2.5 rounded-full shrink-0 ${cores[nivel] ?? "bg-gray-400"}`} />
      <span className="text-sm">{nivel}</span>
    </div>
  );
}
...
// --- Cabeçalho com tooltip ---
function HeaderComTooltip({ label, colKey }: { label: string; colKey: string }) {
  const dica = TOOLTIPS[colKey];
  if (!dica) return <span>{label}</span>;
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" className="max-w-[200px] text-xs">{dica}</TooltipContent>
    </Tooltip>
  );
}
...
function acaoRecomendada(aluno: Aluno): { titulo: string; descricao: string } {
  const acadAlto   = aluno.Risco_Academico === "Alto";
  const evasaoAlto = aluno.Risco_Evasao    === "Alto";
  if (acadAlto && evasaoAlto)  return { titulo: "🔴 Intervenção Urgente",   descricao: "Risco crítico nos dois índices. Contato imediato da coordenação — avalie encaminhamento para suporte pedagógico e psicossocial." };
  if (!acadAlto && evasaoAlto) return { titulo: "🟠 Risco de Evasão",       descricao: "Notas ok, mas sinais silenciosos de desengajamento. Recomenda-se mentoria para fortalecer o vínculo com a instituição." };
  if (acadAlto && !evasaoAlto) return { titulo: "🟡 Dificuldade Acadêmica", descricao: "Aluno engajado, mas com dificuldades de desempenho. Encaminhe para sessões de reforço ou tutoria acadêmica." };
  return { titulo: "🟢 Saudável", descricao: "Nenhum fator de risco relevante identificado. Manter monitoramento regular." };
}

// --- Fatores de evasão do aluno ---
function fatoresEvasao(aluno: Aluno): { fator: string; valor: string; risco: boolean }[] {
  return [
    { fator: "Motivação",          valor: aluno.Nivel_Motivacao,   risco: aluno.Nivel_Motivacao   === "Baixo" },
    { fator: "Apoio Familiar",     valor: aluno.Envolvimento_Pais, risco: aluno.Envolvimento_Pais === "Baixo" },
    { fator: "Distância",          valor: aluno.Distancia_Casa,    risco: aluno.Distancia_Casa    === "Longe" },
    { fator: "Acesso a Recursos",  valor: aluno.Acesso_Recursos,   risco: aluno.Acesso_Recursos   === "Baixo" },
    { fator: "Ativ. Extras",       valor: aluno.Atividades_Extra,  risco: aluno.Atividades_Extra  === "Não"   },
  ];
}
...
const columns: ColumnDef<Aluno>[] = [
  {
    accessorKey: "ID_Aluno",
    header: makeHeader("ID", "ID_Aluno"),
    cell: ({ row }) => <span className="text-muted-foreground text-xs">#{row.original.ID_Aluno}</span>,
    enableHiding: false,
  },
  {
    accessorKey: "Nivel_Risco",
    header: makeHeader("Risco Geral", "Nivel_Risco"),
    cell: ({ row }) => <RiscoBadge nivel={row.original.Nivel_Risco} />,
    filterFn: (row, _, value) => value === "Todos" || row.original.Nivel_Risco === value,
  },
  {
    accessorKey: "Probabilidade_Problema",
    header: makeHeader("Score Geral", "Probabilidade_Problema"),
  ...
  },
  {
    ...
    header: makeHeader("Risco Acadêmico", "Risco_Academico"),
    cell: ({ row }) => <RiscoBadge nivel={row.original.Risco_Academico} />,
    filterFn: (row, _, value) => value === "Todos" || row.original.Risco_Academico === value,
  },
  {
    accessorKey: "Score_Academico",
    header: makeHeader("Score Acadêmico", "Score_Academico"),
  ...
  },
  {
    ...
    header: makeHeader("Risco Evasão", "Risco_Evasao"),
    cell: ({ row }) => <RiscoBadge nivel={row.original.Risco_Evasao} />,
    filterFn: (row, _, value) => value === "Todos" || row.original.Risco_Evasao === value,
  },
  {
    accessorKey: "Score_Evasao",
    header: makeHeader("Score Evasão", "Score_Evasao"),
...
export function DataTableAlunos({ data }: { data: Aluno[] }) {
  ...
  React.useEffect(() => {
    const filters: ColumnFiltersState = [];
    if (filtroGeral     !== "Todos") filters.push({ id: "Nivel_Risco",     value: filtroGeral });
    if (filtroAcademico !== "Todos") filters.push({ id: "Risco_Academico", value: filtroAcademico });
    if (filtroEvasao    !== "Todos") filters.push({ id: "Risco_Evasao",    value: filtroEvasao });
    setColumnFilters(filters);
    setPagination((p) => ({ ...p, pageIndex: 0 }));
  }, [filtroGeral, filtroAcademico, filtroEvasao]);
  ...
  const colunasPodeSocultar = table.getAllColumns().filter((c) => c.getCanHide());
  const visiveisCount = colunasPodeSocultar.filter((c) => c.getIsVisible()).length;
  const acao = alunoSelecionado ? acaoRecomendada(alunoSelecionado) : null;
  const fatores = alunoSelecionado ? fatoresEvasao(alunoSelecionado) : [];

  function FiltroDropdown({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="gap-1.5">
            {label}
            {value !== "Todos" && (
              <span className="rounded-full bg-primary text-primary-foreground text-xs px-1.5 py-0.5 leading-none">{value}</span>
            )}
            <IconChevronDown className="size-3.5 opacity-50" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="w-36">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuRadioGroup value={value} onValueChange={onChange}>
            {NIVEIS.map((n) => <DropdownMenuRadioItem key={n} value={n}>{n}</DropdownMenuRadioItem>)}
          </DropdownMenuRadioGroup>
        </DropdownMenuContent>
      </DropdownMenu>
    );
  ...
  return (
    <>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 px-4 lg:px-6">
          <div className="flex flex-wrap gap-2">
            <FiltroDropdown label="Risco Geral"     value={filtroGeral}     onChange={setFiltroGeral} />
            <FiltroDropdown label="Risco Acadêmico" value={filtroAcademico} onChange={setFiltroAcademico} />
            <FiltroDropdown label="Risco Evasão"    value={filtroEvasao}    onChange={setFiltroEvasao} />
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">
            ...
            <DropdownMenuContent align="end" className="w-52">
              {colunasPodeSocultar.map((col) => {
                ...
                return (
                  ...
                  >
                    {col.id.replace(/_/g, " ")}
                    {seriaUltima && <span className="ml-auto text-xs text-muted-foreground">mín. 1</span>}
                  </DropdownMenuCheckboxItem>
                );
              })}
            </DropdownMenuContent>
              ...
              {table.getRowModel().rows.length ? (
                table.getRowModel().rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <Button variant="outline" size="sm" onClick={() => setAlunoSelecionado(row.original)}>
                        Ver aluno
                      </Button>
                    </TableCell>
                  </TableRow>
        ...
        <div className="flex items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-1">
              <Button variant="outline" size="icon" className="size-8" onClick={() => table.setPageIndex(0)} disabled={!table.getCanPreviousPage()}><IconChevronsLeft /></Button>
              ...
              <Button variant="outline" size="icon" className="size-8" onClick={() => table.setPageIndex(table.getPageCount() - 1)} disabled={!table.getCanNextPage()}><IconChevronsRight /></Button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Drawer */}
      <Drawer open={!!alunoSelecionado} onOpenChange={(open) => !open && setAlunoSelecionado(null)} direction="right">
        <DrawerContent>
          {alunoSelecionado && acao && (
            <>
              <DrawerHeader className="gap-1">
                <DrawerTitle>Aluno #{alunoSelecionado.ID_Aluno}</DrawerTitle>
                <DrawerDescription className="text-xs">{alunoSelecionado.Texto_Diagnostico}</DrawerDescription>
              </DrawerHeader>
              <div className="flex flex-col gap-4 overflow-y-auto px-4">
                ...
                {(() => {
                  ...
                  console.log(
                    alunoSelecionado.Risco_Motivacao,
                    typeof alunoSelecionado.Risco_Motivacao,
                  );
                  const radialData: RadialItem[] = [
                    ...
                    },
                    {
                      ...
                      valor: a.Risco_Tendencia === -1 ? 5 : a.Risco_Tendencia === 0 ? 20 : 50,
                      fill:  a.Risco_Tendencia === -1 ? "#10b981" : a.Risco_Tendencia === 0 ? "#f97316" : "#f43f5e",
                    },
                  ];
                  return (
                    <div className="flex flex-col gap-1">
                      <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Perfil de Risco</div>
                      <ChartRadialLabel data={radialData} />
                      <p className="text-xs text-center text-muted-foreground"><span style={{color:"#f43f5e"}}>■</span> risco alto &nbsp; <span style={{color:"#f97316"}}>■</span> atenção &nbsp; <span style={{color:"#10b981"}}>■</span> ok</p>
                    </div>
                  );
                })()}
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-sm">
                  {([
                    ["Geral",     `${alunoSelecionado.Probabilidade_Problema}%`, alunoSelecionado.Nivel_Risco],
                    ["Acadêmico", `${alunoSelecionado.Score_Academico}%`,        alunoSelecionado.Risco_Academico],
                    ["Evasão",    `${alunoSelecionado.Score_Evasao}%`,           alunoSelecionado.Risco_Evasao],
                  ] as [string, string, string][]).map(([label, score, nivel]) => (
                    <div key={label} className="rounded-lg border p-3 flex flex-col items-center gap-1">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-1 gap-1.5">
                    {fatores.map(({ fator, valor, risco }) => (
                      <div key={fator} className={`flex items-center justify-between rounded-lg border px-3 py-2 text-sm ${risco ? "border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30" : ""}`}>
                        <span className="text-muted-foreground">{fator}</span>
                        <div className="flex items-center gap-1.5">
                          <span className={`inline-block size-2 rounded-full ${risco ? "bg-red-500" : "bg-green-500"}`} />
                          <span className="font-medium">{valor}</span>
                        </div>
                      </div>
                    ))}
